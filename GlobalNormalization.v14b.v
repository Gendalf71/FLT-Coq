...
(*
  Clarified reading of (2.10)--(2.17): the odd binomial core admits
  two real normalisations.  First, core = n * l^n extracts the linear
  factor n.  Second, core = 2^(n-1) * q^n extracts the total odd
  binomial-coefficient mass.  These are real scale factorisations, not
  integer-divisibility assertions about the core.
*)
Local Open Scope R_scope.
...
Proof.
  ...
  { pose proof Hl as Hleft.
    ...
    rewrite Hleft in Hq.
    exact Hq. }
  assert (Hqpow_pos : 0 < pow q n).
  { clear core l Hl Hq Hsym Heq.
    induction n as [|n IH]; simpl.
    - lra.
    - apply Rmult_lt_0_compat; [exact Hqpos|exact IH]. }
...
Lemma mutual_scale_data_excludes_high_exponent (s : MutualScaleData) :
  (2 < ms_n s)%nat -> False.
Proof.
  intro Hgt.
